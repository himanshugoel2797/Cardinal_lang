...
( cd "$comp" && python3 "$root/bootstrap/cardinal.py" emitx86.cardinal "$comp/emitir.cardinal" "$root/lib" ) > "$tmp/xcc.s"
...
( cd "$comp" && "$tmp/xcc" "$comp/emitir.cardinal" "$root/lib" ) > "$tmp/by_x86.c"
( cd "$comp" && python3 "$root/bootstrap/cardinal.py" emitir.cardinal "$comp/emitir.cardinal" "$root/lib" ) > "$tmp/by_py.c"

...
class Checker:
    ...
    def _check_to_str_hook(self, d, sig):
        # DESIGN §5.5/§5.6: `<T>_to_str` for a local type T is the override hook
        # and MUST be (T) -> str (else it collides with the autogenerated hook).
        name = d.name
        ...
        if not (okp and okr):
            self.err(f"to_str hook {name} must have signature ({tn}) -> str", d)

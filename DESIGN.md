...
- **Per-user-type hook `T_str`.** For every user type `T` (struct / enum / sum)
  defined in module `M`, the conversion is the free function
  `func T_str (x T) -> str` (name provisional — `<bare-type-name>_str`). The
...
- **The override lives in the type's defining module** (§10 module-local types)
  ...
  always calls the same `mod::T_str`, so a `pb::Point` formats identically from
...
- **`to_str` dispatch:**
  ...
  - *user types* → call `T_str(x)` (autogen or override).

- **Autogenerated `T_str` bodies (the canonical forms, = today's `_display`):**
...
- [ ] **Float format.** `f32`/`f64` `to_str` is currently inconsistent
  (interpreter Python `repr` vs compiler `%g`, e.g. `1.0` vs `1`). This section is
  the place to pin ONE rule used by both backends. Recommendation: define it as
  the shortest decimal string that round-trips (Ryū/Grisu-style), or, as a
  simpler interim, a single fixed `printf` format (e.g. `%g`) mirrored in the
  interpreter. Decide the rule.
- [ ] Hook **naming** bikeshed (`T_str` vs `T_to_str` vs `show_T`) and whether
  `to_str` is the right name for the generic (vs `str`, which visually collides
  with the type).
- [ ] Should an **override be required to round-trip / be side-effect-free**, or
  is it free-form? (Display is observable; a misbehaving override is the user's
  problem, but worth a stated expectation.)
- [ ] `to_str` of a **closure / foreign handle** — opaque form (`<closure>`?) or
  a compile error? (Rare; `_display` currently falls back to a host repr.)
- [ ] **Generation strategy** (impl note, not language-observable): generate
  `T_str` lazily — only for types reached by a `to_str`/print site or transitively
  by another generated hook — to avoid emitting unused conversions.
...
- [~] Value display / stringification — *type-directed `to_str` builtin + autogenerated, overridable per-type `T_str` hook in the type's defining module (§5.5). Model decided; float format + naming still open.*

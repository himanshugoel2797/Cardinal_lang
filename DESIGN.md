...
**Open (need direction — see discussion):**

- [ ] **Runtime reflection?** Whether to ALSO provide runtime type descriptors
  (introspect a value dynamically: enumerate fields, fetch the type name) to
  enable generic runtime code (dynamic serializers, debuggers). Cost: metadata in
  the binary + a step away from zero-cost. *Lean: compile-time only to start;
  runtime descriptors opt-in later if a concrete need appears.*
- [ ] **User-facing metaprogramming?** Whether users can write their OWN derivers
  via a `comptime`/reflection API (iterate a type's fields in Cardinal at compile
  time, à la Zig `comptime`), vs a fixed compiler-owned set of derivable ops.
  *Lean: fixed-but-extensible set first; a general comptime API is a large,
  separate feature to weigh after the language self-hosts.*
- [ ] **Automatic vs opt-in per operation.** `to_str` is always-on. Operations
  with cost or ambiguity (`hash`, `serialize`) probably want **opt-in** via a
  `derive`-style declaration on the type (e.g. `derive eq hash` — syntax TBD) so
  you don't pay for unused derivations and ambiguous ones are explicit.
- [ ] **Wrong-signature diagnostic.** When a module has a function with a hook's
  conventional name but the wrong signature: error (likely a mistaken override) or
  treat as unrelated and emit the canonical anyway? *Lean: error, to catch typos.*
...
- [~] Reflection / derived operations — *compile-time type introspection driving derived ops (autogen + override + always-emit); `to_str` is the first. Runtime reflection and user-facing `comptime` metaprogramming are open forks (§5.6).*

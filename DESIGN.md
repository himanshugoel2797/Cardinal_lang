...
### 5.5 Stringification (`to_str`) & value display

> Replaces the hardwired display logic (interpreter `_display` / compiler
> `lower_display`) with a type-directed, **autogenerated, overridable** string
> conversion. The motivating constraint: recursive types (sum types — an AST node
> contains AST nodes) cannot be displayed by recursing over the *type* at compile
> time; they need a per-type function that recurses over the *data* at runtime.

**Decided (this round):**

- **`to_str` — a type-dispatched builtin** `(to_str x) -> str`, resolved by `x`'s
  static type (the same way `len`/`push`/`map_keys` are special-cased — *not*
  general overloading). It is the single entry point for "the string form of a
  value", and what `io::print`/`io::println` call on each argument
  (`io::print x ≡ print the bytes of (to_str x)`). So printing and conversion
  share one definition; the output of a no-override program is byte-for-byte what
  `_display` produces today.

- **Per-user-type hook `T_to_str`.** For every user type `T` (struct / enum / sum)
  defined in module `M`, the conversion is the free function
  `func T_to_str (x T) -> str` (`<bare-type-name>_to_str`). The
  compiler **autogenerates** it **unless `M` already defines a function of that
  exact name and signature `(T) -> str`**, in which case the user's version is
  used verbatim — that is the override. No overloading is introduced: each type's
  hook has a distinct name, so a module may override several types.

- **The override lives in the type's defining module** (§10 module-local types)
  and is **exported** (autogenerated hooks are implicitly exported; an override is
  treated as part of the type's public contract). Cross-module display therefore
  always calls the same `mod::T_to_str`, so a `pb::Point` formats identically from
  every call site — formatting travels with the type, not the use site.

- **`to_str` dispatch:**
  - *scalars* (`i*`/`u*`/`f*`/`bool`/`char`/`str`/`unit`/`null`) → fixed builtin
    forms: integers decimal; `bool`→`true`/`false`; `char`→the UTF-8 character;
    `str`→its own bytes (identity, no quotes); `unit`→`unit`; `null`→`null`.
    These are **not** overridable.
  - *containers* (`[T]` array, `{T}` vector, `{K V}` map) → **structural**,
    recursing through `to_str` on elements/keys/values: array `[e0 e1 …]`,
    vector `{e0 e1 …}`, map `{k0: v0 k1: v1 …}` (insertion order), single-space
    separated. Containers are structural (not per-instantiation overridable);
    overriding an element's *type* changes how that element renders.
  - *user types* → call `T_to_str(x)` (autogen or override).

- **Autogenerated `T_to_str` bodies (the canonical forms, = today's `_display`):**
  - struct → `(TypeName f0: <to_str field0> f1: <to_str field1> …)` — bare type
    name, fields in **declaration order**, space-separated.
  - enum → a switch on the value yielding `TypeName::Variant`.
  - sum → a `match` on the variant: `(Variant f: <to_str payload> …)`, nullary
    `(Variant)`. (No type name — just the variant, matching `_display`.)
  Because an autogenerated body calls `to_str` on its fields/payloads, and
  `to_str` of a user type calls that type's hook, **recursion happens at runtime
  over the data** and terminates on finite values — this is what makes recursive
  sum types displayable.

- **Both front ends implement identical autogen + override-resolution** (the
  interpreter's `_display` becomes "call the override if the defining module
  has one, else the canonical form"), so native output stays byte-identical to
  the interpreter (the parity gate). The override-lookup rule must be
  verdict-equivalent in `checker.cardinal` and `typecheck.py`.

**Decided (resolved):**

- **Float format = C `%g`**, used by BOTH backends. The interpreter is tightened
  from Python `repr` to `%g`-equivalent so native and oracle agree (a defined,
  if lossy, rule; a shortest-round-trip refinement can come later without changing
  the contract that both backends match).
- **Naming:** generic `to_str`; per-type hook `<Type>_to_str`.
- **Overrides are free-form** — an override may produce any string; correctness /
  round-tripping is the user's responsibility (display is observable output, not
  a parsed format).
- **`to_str` of a closure → `<closure>`**; a foreign `handle` → `<handle>`
  (opaque, never a compile error).
- **Always emit** every type's `T_to_str` (eager, not lazy): the hooks are part of
  a type's public contract, so they must exist when the module is consumed as a
  **library** (a downstream module may display a type this module never prints).
  Exported like any other public function.

### 5.6 Reflection & derived operations

> §5.5's autogenerated `to_str` is one instance of a more general capability: the
> compiler **reflects over a type's structure** (fields, variants, names — §10
> module-local identity) and emits code from it. Cardinal makes this a first-class
> concept — *derived operations* — rather than a one-off for printing.

**Decided (the foundation):**

- **Reflection is compile-time / static.** The compiler introspects type structure
  at compile time and emits concrete functions; there is **no mandatory runtime
  type metadata** (no RTTI, no per-value type tags beyond the discriminant sum
  types already carry). A derived operation is an ordinary, zero-cost emitted
  function — consistent with the systems ethos (§1).
- **The introspection substrate already exists** inside the compiler: the
  checker's type tables (struct fields, enum variants, sum variants, qualified
  identity). "Reflection" is formalizing the use of that structure to *generate
  code*, not a new runtime facility.
- **Derived-operation pattern** (generalizes §5.5). For a derivable operation
  `op`, each user type `T` (in its defining module) gets a canonical
  `T_op` function computed from `T`'s structure, **overridable** by a same-named
  user function with the matching signature, **always emitted and exported**.
  `to_str` is the first member. The compiler resolves, per `(type, op)`: use the
  user's hook iff a function of the conventional name **and exact expected
  signature** exists in the type's module; otherwise emit the canonical body.
- **Roadmap of derivable operations** (each added deliberately, with its canonical
  form specified when introduced): `to_str` (done in spec, §5.5); `eq` / `cmp`
  (structural equality / ordering); `hash` (hashing for map keys & future sets);
  `clone` (deep copy — mostly redundant with value-type copy, relevant once
  reference fields nest); `serialize` / `deserialize`. Sum/recursive types work
  because, like `to_str`, a derived body calls the operation on its
  fields/payloads and recurses over the **data** at runtime.

**Decided (this round):**

- **Runtime reflection is IN scope** (not compile-time only). Cardinal will carry
  **runtime type descriptors** so a value can be introspected dynamically
  (enumerate fields, fetch the type name), enabling generic runtime code (dynamic
  serializers, debuggers, a generic `to_str` that walks descriptors). This is its
  own subsystem with a design TBD (descriptor layout, where the table lives, how
  much metadata, opt-in vs always); it is **not required by `to_str`**, which
  ships now via static autogen and stays forward-compatible (the per-type hook is
  the stable contract whether dispatch is static or descriptor-driven).
- **Fixed, compiler-owned set of derivable operations** (no user-facing
  `comptime` metaprogramming for now). The compiler knows how to derive an
  extensible list (`to_str`, then `eq`/`cmp`/`hash`/…); users customize via the
  per-type override hook, not by writing their own derivers. A general comptime
  API is reconsidered only after the language self-hosts.
- **Per-operation automatic vs opt-in:** `to_str` is always-on; costly/ambiguous
  ops (`hash`, `serialize`) will be **opt-in** via a `derive`-style declaration on
  the type (syntax TBD) so unused derivations aren't paid for.
- **Wrong-signature hook = error.** A function with a hook's conventional name but
  the wrong signature is a compile error (a typo'd override), not silently
  ignored.
...
- [ ] C backend gaps (self-hosting Cardinal backend, `compiler/`): **sum types +
  `match`** (tagged-union emission + tag switch) and **closures** (the last
  Stage-4 work), `null`/handles/`ref`, `for`-loop-variable capture. DONE: value
  display (`to_str` + `io::print` of struct/array/vector/map/enum — the canonical
  `_display` form, recursive, with overridable per-type `<Type>_to_str` hooks,
  §5.5); strings; vectors; maps.
...
- [x] Value display / stringification — *type-directed `to_str` builtin + autogenerated, overridable, always-emitted per-type `<Type>_to_str` hook in the type's defining module; floats via `%g`; `<closure>`/`<handle>` opaque (§5.5).*
- [~] Reflection / derived operations — *compile-time type introspection driving a fixed compiler-owned set of derived ops (autogen + override + always-emit); `to_str` is the first. Runtime type descriptors are IN scope (own subsystem, design TBD); no user-facing `comptime`. (§5.6)*
